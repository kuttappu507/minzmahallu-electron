...
function ProtectedLayout() {
  ...
  // Navigating always closes the phone drawer (a page change is the "done" signal).
  useEffect(() => { setNavOpen(false); }, [location.pathname, setNavOpen]);
  return <div id="app" className={navOpen ? "app-shell nav-open" : "app-shell"}><Topbar /><div className="app-body"><div className="mms-nav-backdrop" onClick={() => setNavOpen(false)} aria-hidden="true" /><Sidebar /><div className="maincol"><div id="content"><Suspense fallback={<div className="flex items-center justify-center h-64"><div className="spinner-sm" /></div>}><Routes>
...
/** A QR code scanned with the phone opens `mms://verify/<code>`, which the
 *  native shell stores and announces. Following it is only possible once the
 *  user is signed in, so this component waits for the session and then walks
 *  the user to Certificates, where the code is picked up and verified. */
const DEEPLINK_STORAGE_KEY = "mms-pending-verify";
...
function PendingDeepLink() {
  ...
  useEffect(() => {
    if (!user) return;
    let pending = "";
    try { pending = window.sessionStorage.getItem(DEEPLINK_STORAGE_KEY) || ""; } catch { /* ignore */ }
    if (!pending) return;
    if (!location.pathname.startsWith("/certificates")) navigate("/certificates");
...
export default function App() {
  ...
  return <><PendingDeepLink /><LanguagePersistence /><OfflineMalayalamLayer /><Routes><Route path="/login" element={user ? <Navigate to="/" /> : <LoginPage />} /><Route path="/*" element={user ? <ProtectedLayout /> : <Navigate to="/login" />} /></Routes><ToastContainer /><UpdateBanner />{!splashDone && <Splash onDone={() => setSplashDone(true)} />}<CloseConfirmDialog /></>;
}
