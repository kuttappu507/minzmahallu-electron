...
describe("device fingerprint (anti-forgery binding)", () => {
  it("is deterministic on the same machine", async () => {
    expect(await computeDeviceFingerprint()).toBe(await computeDeviceFingerprint());
    expect(await computeDeviceFingerprint()).toMatch(/^[0-9A-F]{16}$/);
  });

  it("differs when hardware identity changes", async () => {
    const parts = await deviceIdParts();
    // Swapping one identity part must change the hash.
    const a = await computeDeviceFingerprint();
    const other = parts.map((p) => (p.startsWith("host:") ? "host:other-phone" : p)).join("|");
    const { sha256Hex } = await import("../platform/crypto.js");
    const b = (await sha256Hex(other)).slice(0, 16).toUpperCase();
    expect(b).not.toBe(a);
  });
