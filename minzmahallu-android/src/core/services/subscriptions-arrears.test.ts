...
describe("WhatsApp receipt send-lock — only DELIVERY locks (V037)", () => {
  ...
  it("an unknown confirmation never locks another receipt (no false already-sent)", () => {
    // Android hands the PDF over through the share sheet, so the lock is set at
    // hand-off — but a stray/unknown confirmation id must never mark some other
    // receipt as delivered.
    const delivered = () =>
      Number((getDB().prepare("SELECT COUNT(*) AS c FROM subscription_payments WHERE receipt_delivered_at IS NOT NULL").get() as any).c);
    const before = delivered();
    markReceiptDeliveredByMsgId("MSG-NEVER-SENT");
    expect(delivered()).toBe(before);
    expect(() => markReceiptDeliveredByMsgId("")).not.toThrow();
  });
});
