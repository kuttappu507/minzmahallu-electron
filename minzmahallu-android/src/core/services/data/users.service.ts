...
export const users = {
  ...
  create: async (data: any, creatorRole: string) => {
    ...
    const { stored, salt } = await hashPasswordForStorage(data.password);
    const { id } = run(
  ...
  resetPassword: async (id: number, newPassword: string) => {
    const { stored, salt } = await hashPasswordForStorage(newPassword);
