...
/*
 * Secured API surface — the heart of the app's access control.
 *
 * Every privileged operation (archiving, corrections of financial records,
 * voids, cancellations, disbursements, user management, exports) is reachable
 * only through the handlers below: each one demands a logged-in actor, and the
 * sensitive ones demand an explicit administrator re-authentication. On the
 * desktop these were IPC channels; on Android they are plain registered
 * functions called by the bridge — the checks are identical because this file
 * IS the desktop file, with only the transport swapped out.
 */
...
export function registerSecurityIpc(register: IpcRegistrar, getActor: ActorProvider) {
  register("auth:setupStatus", () => ({ required: needsInitialSetup() }));
  register("auth:createInitialAdministrator", async (username: string, fullName: string, password: string) => {
    const user = await createInitialAdministrator(username, fullName, password);
  ...
  register("auth:changePassword", async (userId: number, newPassword: string) => {
    const a = actor();
    if (userId !== a.id && a.role !== "Administrator") throw new Error("You can only change your own password");
    validatePassword(newPassword); await changePassword(userId, newPassword);
  ...
  register("auth:verifyAdminPassword", async (password: string, action = "secure action", detail = "") => {
    const a = actor();
    const verified = await verifyCurrentActorPassword(String(password ?? ""));
  ...
  register("subscriptions:cancelPayment", async (id: number, reason: string, adminPassword: string) => {
    const a = admin();
    await verifyCurrentActorPassword(String(adminPassword ?? ""));
  ...
  register("donations:update", async (id: number, d: any, adminPassword: string, reason: string) => {
    const a = admin();
  ...
  register("accounting:update", async (id: number, d: any, adminPassword: string, reason: string) => {
    const a = admin();
  ...
  register("accounting:void", async (id: number, reason: string, adminPassword: string) => {
    const a = admin();
  ...
  register("welfare:disburse", async (id: number, reason = "", adminPassword: string) => {
    const a = admin();
    await verifyCurrentActorPassword(String(adminPassword ?? ""));
  ...
  register("users:create", async (d: any) => { const a = admin(); validatePassword(String(d?.password ?? "")); return await data.users.create(d, a.role); });
  ...
  register("users:resetPassword", async (id: number, p: string) => { const a = admin(); validatePassword(p); await changePassword(id, p); try { data.audit.log(a.id, a.username, "PASSWORD_RESET", "users", id, "Administrator reset user password", ""); } catch {} return { success: true }; });
  ...
  register("staff:setStatus", async (id: number, status: "Resigned" | "Expelled", effectiveDate: string, reason: string, adminPassword: string) => {
    const a = admin();
    await verifyCurrentActorPassword(String(adminPassword ?? ""));
  ...
  register("staff:cancelPayment", async (id: number, reason = "", adminPassword: string) => {
    const a = admin();
    await verifyCurrentActorPassword(String(adminPassword ?? ""));
