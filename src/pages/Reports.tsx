...
function buildXlsx(rows:any[],c:string[],r?:ReportType,ml=false){
  const encoder=new TextEncoder();
  const sheetRows=[`<row r="1">${c.map((k,i)=>xmlCell(`${excelCol(i)}1`,colLabel(r as ReportType,k,ml),true)).join("")}</row>`];
  rows.forEach((row,ri)=>{const rr=ri+2;sheetRows.push(`<row r="${rr}">${c.map((k,ci)=>xmlCell(`${excelCol(ci)}${rr}`,mlVal(istCell(row[k])))).join("")}</row>`);});
