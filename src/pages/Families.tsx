...
export function Families() {
  ...
  const openPreview=async(r:Family)=>{setPreviewRow(r);setPreviewOpen(true);setMembers([]);try{const[hist,mem]=await Promise.all([window.mms.families.history(r.id),window.mms.members.list({familyId:r.id,pageSize:1000})]);setHistory(hist);setMembers(mem.rows||[]);}catch{setHistory([]);setMembers([]);}};
