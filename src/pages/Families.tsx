...
import { statusVariant, formatDateTime } from "@/lib/utils";
...
export function Families() {
  ...
  return <div className="view view-enter">
    ...
    <Dialog open={previewOpen} onClose={()=>setPreviewOpen(false)} title={t("family_title")}>
      ...
      <div className="dlg-sec"><b>{tx("Household", "കുടുംബം")}</b></div>
      <div className="det-grid">{[[t("family_number"),previewRow.family_number],[t("family_house_name"),previewRow.house_name],[t("family_house_number"),previewRow.house_number||"—"],[t("family_ward"),previewRow.ward||"—"],[t("family_area"),previewRow.area||"—"],[t("family_members_count"),String(previewRow.member_count||0)]].map(([k,v],i)=><div className="det" key={i}><span className="k">{k}</span><span className="v">{v}</span></div>)}</div>
      <div className="dlg-sec"><b>{tx("Contact & Location", "ബന്ധപ്പെടാനുള്ള വിവരങ്ങളും വിലാസവും")}</b></div>
      <div className="det-grid">{[[t("family_phone"),previewRow.phone],[t("family_alt_phone"),previewRow.alt_phone||"—"],[tx("WhatsApp number","വാട്ട്സ്ആപ്പ് നമ്പർ"),previewRow.whatsapp_phone||tx("Not added","ചേർത്തിട്ടില്ല")],[tx("WhatsApp messaging","വാട്ട്സ്ആപ്പ് സന്ദേശങ്ങൾ"),previewRow.whatsapp_enabled?tx("Allowed","അനുവദിച്ചു"):tx("Disabled","അനുവദിച്ചിട്ടില്ല")],[t("family_pincode"),previewRow.pincode||"—"],[t("family_address"),previewRow.address||"—"],[t("family_notes"),previewRow.notes||"—"]].map(([k,v],i)=><div key={i} className={`det${i>=5?" full":""}`}><span className="k">{k}</span><span className="v">{v}</span></div>)}</div>
      <div className="mt-5"><div className="flex items-center gap-2 mb-3"><Users size={16}/><strong>{t("family_members_list")}</strong><Badge variant="muted">{members.length}</Badge></div><div className="space-y-2 max-h-56 overflow-auto">{members.length?members.map((m,i)=><div key={m.id||i} className="p-3 rounded-lg border border-border flex items-center gap-3"><span className="w-8 h-8 rounded-full bg-surface-hover grid place-items-center text-xs font-semibold text-muted">{(m.name||"?").charAt(0).toUpperCase()}</span><div className="flex-1 min-w-0"><div className="font-medium truncate">{m.name||"—"}</div><div className="text-xs text-muted">{m.member_code}{m.relationship?` · ${m.relationship}`:""}{m.mobile?` · ${m.mobile}`:""}</div></div>{m.is_head===1&&<Badge variant="success">{tx("Head","കുടുംബനാഥൻ")}</Badge>}</div>):<div className="text-sm text-muted">{t("family_no_members")}</div>}</div></div>
      <div className="mt-5"><div className="flex items-center gap-2 mb-3"><History size={16}/><strong>{tx("Activity & History","പ്രവർത്തനങ്ങളും ചരിത്രവും")}</strong></div><div className="space-y-2 max-h-56 overflow-auto">{history.length?history.map(h=>{const changes=formatHistoryChanges(h.changes_json);return <div key={h.id} className="p-3 rounded-lg border border-border"><div className="flex justify-between gap-3"><b>{h.summary}</b><span className="text-xs text-muted">{formatDateTime(h.changed_at)}</span></div><div className="text-xs text-muted mt-1">{h.username} · {h.action}{h.reason?` · ${h.reason}`:""}</div>{changes.length>0&&<div className="mt-2 space-y-1">{changes.map((c,i)=><div key={i} className="grid grid-cols-[minmax(90px,0.8fr)_minmax(0,1fr)_minmax(0,1fr)] gap-2 text-xs"><span className="font-medium capitalize">{c.field}</span><span className="rounded bg-muted/30 px-2 py-1 break-words">{c.oldValue}</span><span className="rounded bg-muted/30 px-2 py-1 break-words">{c.newValue}</span></div>)}</div>}{h.reason&&!changes.length&&<div className="text-sm mt-2">{h.reason}</div>}</div>}) : <div className="text-sm text-muted">{tx("No history recorded","ചരിത്ര രേഖകളില്ല")}</div>}</div></div></>}
    ...
    <Dialog open={securityOpen} onClose={()=>setSecurityOpen(false)} title={pendingAction==="archive"?tx("Archive family","കുടുംബം ആർക്കൈവ് ചെയ്യുക"):tx("Restore family","കുടുംബം പുനഃസ്ഥാപിക്കുക")} className="modal-sm">
      <div className="p-6 space-y-4"><p>{pendingAction==="archive"?tx("All active members of this family will also be archived. Their records and history will be preserved.","ഈ കുടുംബത്തിലെ എല്ലാ സജീവ അംഗങ്ങളും ആർക്കൈവ് ചെയ്യപ്പെടും. അവരുടെ രേഖകളും ചരിത്രവും സംരക്ഷിക്കപ്പെടും."):tx("Only members archived because of this family archive will be restored.","ഈ കുടുംബം ആർക്കൈവ് ചെയ്തതിനെ തുടർന്ന് ആർക്കൈവ് ചെയ്ത അംഗങ്ങളെ മാത്രം പുനഃസ്ഥാപിക്കും.")}</p><div><Label>{tx("Reason","കാരണം")}{pendingAction==="archive"?" *":""}</Label><Textarea rows={3} value={reason} onChange={e=>setReason(e.target.value)} placeholder={tx("Why is this action being performed?","ഈ പ്രവർത്തനം നടത്താനുള്ള കാരണം?")}/></div><div className="flex justify-end gap-2"><Button variant="secondary" onClick={()=>setSecurityOpen(false)}>{t("action_cancel")}</Button><Button onClick={executeSecurity}>{pendingAction==="archive"?tx("Archive family","കുടുംബം ആർക്കൈവ് ചെയ്യുക"):tx("Restore family","കുടുംബം പുനഃസ്ഥാപിക്കുക")}</Button></div></div>
    </Dialog>

    <Dialog open={dialogOpen} onClose={()=>setDialogOpen(false)} title={editingId?t("action_edit"):t("add_family")} className="max-w-2xl"><div className="p-6 space-y-4"><div className="grid grid-cols-2 gap-4"><div><Label>{t("family_house_name")} *</Label><Input value={form.house_name||""} onChange={e=>setForm({...form,house_name:e.target.value})}/></div><div><Label>{t("family_house_number")}</Label><Input value={form.house_number||""} onChange={e=>setForm({...form,house_number:e.target.value})}/></div><div><Label>{t("family_ward")}</Label><Input value={form.ward||""} onChange={e=>setForm({...form,ward:e.target.value})}/></div><div><Label>{t("family_area")}</Label><Input value={form.area||""} onChange={e=>setForm({...form,area:e.target.value})}/></div><div><Label>{t("family_phone")} * (10 {tx("digits","അക്കങ്ങൾ")})</Label><Input value={form.phone||""} onChange={e=>{const d=clampPhone10(e.target.value);setForm({...form,phone:d});}} inputMode="numeric" placeholder="98XXXXXXXX"/></div><div><Label>{t("family_alt_phone")} (10 {tx("digits","അക്കങ്ങൾ")})</Label><Input value={form.alt_phone||""} onChange={e=>{const d=clampPhone10(e.target.value);setForm({...form,alt_phone:d});}} inputMode="numeric"/></div><div><Label>{t("family_pincode")}</Label><Input value={form.pincode||""} onChange={e=>setForm({...form,pincode:e.target.value})}/></div></div><div className="rounded-lg border border-border bg-surface-muted p-4 space-y-3"><div className="flex items-center gap-2"><MessageCircle className="h-4 w-4 text-primary"/><strong>{tx("WhatsApp","വാട്ട്സ്ആപ്പ്")}</strong></div><div><Label>{tx("Family head WhatsApp number","കുടുംബനാഥന്റെ വാട്ട്സ്ആപ്പ് നമ്പർ")}</Label><Input value={form.whatsapp_phone||""} onChange={e=>{const d=e.target.value.replace(/\D/g,'').slice(0,15);setForm({...form,whatsapp_phone:d});}} maxLength={15} inputMode="numeric" placeholder="9198XXXXXXXX"/><div className="text-xs text-muted mt-1">{tx("Bulk messages are sent only to the family head.","ബൾക്ക് സന്ദേശങ്ങൾ കുടുംബനാഥന് മാത്രം അയയ്ക്കും.")}</div></div><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.whatsapp_enabled!==0} onChange={e=>setForm({...form,whatsapp_enabled:e.target.checked?1:0})}/>{tx("Allow Mahallu WhatsApp messages","മഹല്ല് വാട്ട്സ്ആപ്പ് സന്ദേശങ്ങൾ അനുവദിക്കുക")}</label></div><div><Label>{t("family_address")}</Label><Textarea rows={2} value={form.address||""} onChange={e=>setForm({...form,address:e.target.value})}/></div><div><Label>{t("family_notes")}</Label><Textarea rows={2} value={form.notes||""} onChange={e=>setForm({...form,notes:e.target.value})}/></div><div className="flex justify-end gap-2"><Button variant="secondary" onClick={()=>setDialogOpen(false)}>{t("action_cancel")}</Button><Button onClick={save} disabled={busy}>{busy ? t("ui_saving") : t("action_save")}</Button></div></div></Dialog>
  </div>;
}