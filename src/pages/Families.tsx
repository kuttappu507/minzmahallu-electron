...
export function Families() {
  ...
  return <div className="view view-enter">
    ...
    <Dialog open={previewOpen} onClose={()=>setPreviewOpen(false)} title={t("family_title")}>
      ...
      <div className="det-grid">{[[t("family_phone"),previewRow.phone],[t("family_alt_phone"),previewRow.alt_phone||"—"],[t("family_pincode"),previewRow.pincode||"—"],[t("family_address"),previewRow.address||"—"],[t("family_notes"),previewRow.notes||"—"]].map(([k,v],i)=><div key={i} className={`det${i>=3?" full":""}`}><span className="k">{k}</span><span className="v">{v}</span></div>)}</div>
