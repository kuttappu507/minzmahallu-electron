...
export function LoginPage() {
  ...
  const copy = {
    bridge: ml ? "MMS ബ്രിഡ്ജ് ലഭ്യമല്ല. പ്രീലോഡ് സ്ക്രിപ്റ്റ് ലോഡ് ചെയ്യുന്നതിൽ പരാജയപ്പെട്ടു." : "MMS bridge not available. The preload script failed to load.", loginFailed: ml ? "ലോഗിൻ പരാജയപ്പെട്ടു. വിവരങ്ങൾ പരിശോധിക്കുക." : "Login failed. Please check your credentials.", unexpected: ml ? "അപ്രതീക്ഷിത പിശക് സംഭവിച്ചു." : "An unexpected error occurred.", welcome: ml ? "സ്വാഗതം" : "Welcome", welcomeBack: ml ? "വീണ്ടും സ്വാഗതം." : "Welcome back.", setupTitle: ml ? "ആദ്യ സജ്ജീകരണം" : "Initial Setup", setupSub: ml ? "ആദ്യ Administrator അക്കൗണ്ട് സൃഷ്ടിക്കുക." : "Create the first Administrator account.", fullName: ml ? "പൂർണ്ണ പേര്" : "Full name", username: ml ? "ഉപയോക്തൃനാമം" : "Username", password: ml ? "പാസ്‌വേഡ്" : "Password", confirm: ml ? "പാസ്‌വേഡ് സ്ഥിരീകരിക്കുക" : "Confirm password", create: ml ? "Administrator അക്കൗണ്ട് സൃഷ്ടിക്കുക" : "Create Administrator Account", creating: ml ? "സൃഷ്ടിക്കുന്നു..." : "Creating...", mismatch: ml ? "പാസ്‌വേഡുകൾ പൊരുത്തപ്പെടുന്നില്ല." : "Passwords do not match.", requirements: ml ? "കുറഞ്ഞത് 8 അക്ഷരങ്ങൾ, വലിയക്ഷരം, ചെറിയക്ഷരം, സംഖ്യ, പ്രത്യേക ചിഹ്നം." : "At least 8 characters, uppercase, lowercase, number and special character.", manageTitle: ml ? "നിങ്ങളുടെ മഹല്ല്\nവ്യക്തതയോടെ നിയന്ത്രിക്കുക." : "Manage your mahallu\nwith clarity.", manageText: ml ? "കുടുംബങ്ങൾ, അംഗങ്ങൾ, സംഭാവനകൾ, സർട്ടിഫിക്കറ്റുകൾ എന്നിവ ഒരിടത്ത് കൈകാര്യം ചെയ്യാനുള്ള ആധുനിക ഡെസ്ക്ടോപ്പ് ആപ്പ്." : "A modern desktop app for families, members, donations, certificates, and more — all in one place.", secured: ml ? "PBKDF2 സുരക്ഷിതം" : "PBKDF2 secured", offline: ml ? "ഓഫ്‌ലൈൻ SQLite" : "Offline SQLite", signing: ml ? "സൈൻ ഇൻ ചെയ്യുന്നു..." : "Signing in...", minimize: ml ? "ചുരുക്കുക" : "Minimize", maximize: ml ? "വലുതാക്കുക" : "Maximize", close: ml ? "അടയ്ക്കുക" : "Close", rotateTitle: ml ? "പാസ്‌വേഡ് പുതുക്കുക" : "Update your password", rotateSub: ml ? "ഈ അക്കൗണ്ട് പൊതുവിൽ അറിയപ്പെടുന്ന ഒരു സ്ഥിര പാസ്‌വേഡ് ആണ് ഉപയോഗിക്കുന്നത്. സുരക്ഷയ്ക്ക് ഇപ്പോൾ പുതിയ പാസ്‌വേഡ് സജ്ജമാക്കുക." : "This account still uses a publicly-known default password. For security, set a new one now.", rotateNew: ml ? "പുതിയ പാസ്‌വേഡ്" : "New password", rotateSaved: ml ? "പാസ്‌വേഡ് പുതുക്കി. സ്വാഗതം!" : "Password updated. Welcome!", rotate: ml ? "പാസ്‌വേഡ് സജ്ജമാക്കുക" : "Set new password"
  };
  ...
  return (
    <div className="login-wrap">
      ...
      <div className="login-left">
        <div className="login-deco-1"/><div className="login-deco-2"/><div className="login-deco-3"/>
        <div className="login-grid-overlay"/>
        <div className="login-pattern" aria-hidden="true"/>
        <div className="login-content">
          <div className="login-logo-row">
            <div className="login-logo-box"><img src="./logo.png" alt="MMS" /></div>
            <div><h1 className="login-brand-name">MMS</h1><p className="login-brand-sub">{t("app_name")}</p></div>
          </div>
          <div>
            <h2 className="login-hero-title">{copy.manageTitle.split("\n").map((line,i)=><span key={i}>{i>0&&<br/>}{line}</span>)}</h2>
            <p className="login-hero-text">{t("app_subtitle")}. {copy.manageText}</p>
            <div className="login-feature-row">
              <div className="login-feature"><span className="login-feature-ic"><ShieldCheck size={14}/></span><b>{copy.secured}</b></div>
              <div className="login-feature"><span className="login-feature-ic"><Database size={14}/></span><b>{copy.offline}</b></div>
            </div>
          </div>
          <div className="login-stats">
            <div className="login-stat"><b>20+</b><small>{t("login_stat_registers")}</small></div>
            <div className="login-stat"><b>100%</b><small>{t("login_stat_offline")}</small></div>
          </div>
        </div>
      </div>
      ...
      <div className="login-right">
        <div className="login-form-wrap">
          <div className="login-mobile-logo-row lg:hidden"><div className="login-logo-box-sm"><img src="./logo.png" alt="MMS" /></div><div><h1 className="login-brand-name">MMS</h1><p className="login-brand-sub">{t("app_name")}</p></div></div>
          {checkedSetup && setup ? <>
