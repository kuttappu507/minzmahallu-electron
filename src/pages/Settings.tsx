...
import { Building2, Wallet, Palette, Database, Save, Tags, Plus, Pencil, Trash2, Power } from "lucide-react";
...
interface Settings {
  ...
  backup_interval_hours:number; subscription_monthly_amount:number;
}
...
const emptySettings: Settings = {
  ...
  subscription_monthly_amount:100,
};
...
export function Settings(){
  ...
  useEffect(()=>{
    Promise.all([window.mms.settings.load(),window.mms.donations.categoriesAll()]).then(([s,c])=>{
      if(s)setSettings({...emptySettings,...s,language:s.language||lang,theme:s.theme||theme,subscription_monthly_amount:Number(s.subscription_monthly_amount??100)});
      setCategories(c||[]);
  ...
  const handleSave=async()=>{
    ...
    try{
      await window.mms.settings.save({
        ...
        subscriptionMonthlyAmount:Number(settings.subscription_monthly_amount||0),theme:settings.theme,
        language:settings.language,autoBackup:settings.auto_backup,backupIntervalHours:settings.backup_interval_hours,
