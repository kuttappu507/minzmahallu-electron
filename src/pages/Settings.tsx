...
import { Building2, Wallet, Palette, Database, Save, Tags, Plus, Pencil, Trash2, Power, Award, MapPin, FolderOpen, Info, RefreshCw } from "lucide-react";
...
interface Settings {
  mahallu_name:string; phone:string; address:string; financial_year_start:string;
...
export function Settings(){
  ...
  const {pref:fxPref,setPref:setFxPref}=useFx();
  const ml=lang==="ml"; const tx=(en:string,m:string)=>ml?m:en;
  ...
  const [appInfo,setAppInfo]=useState<{ version:string }|null>(null);
  const [updState,setUpdState]=useState<"idle"|"checking"|"latest"|"new"|"error">("idle");
  const [updVersion,setUpdVersion]=useState<string>("");
  const [updHasDl,setUpdHasDl]=useState(false);
  const upd=useUpdaterDownload();

  const checkUpdatesNow=async()=>{
    setUpdState("checking");
    try{
      const r=await window.mms.updates.checkNow();
      if(r.ok){ if(r.updateAvailable){setUpdState("new");setUpdVersion(r.latestVersion||"");setUpdHasDl(!!r.downloadUrl);} else setUpdState("latest"); }
      else setUpdState("error");
  ...
  const handleSave=async()=>{
    ...
    try{
      await window.mms.settings.save({
        mahalluName:settings.mahallu_name,address:settings.address,phone:settings.phone,
        ...
        affiliationNumber:settings.affiliation_number,
  ...
  return <div className="p-6 space-y-6">
    ...
    <Card><CardContent className="p-6 space-y-4"><div className="flex items-center gap-2"><Building2 className="h-5 w-5 text-primary"/><SectionLabel className="mb-0">{t("set_org_section")}</SectionLabel></div><div className="grid grid-cols-2 gap-4"><div><Label>{t("set_mahallu_name")}</Label><MalayalamInput value={settings.mahallu_name} onChange={value=>setSettings({...settings,mahallu_name:value})}/><p className="text-xs text-muted mt-1">{t("set_ml_enter_hint")}</p></div><div><Label>{t("set_phone")}</Label><Input value={settings.phone} onChange={e=>{const d=clampPhone10(e.target.value);setSettings({...settings,phone:d});}} inputMode="numeric"/></div><div className="col-span-2"><Label>{t("family_address")}</Label><Textarea rows={2} value={settings.address} onChange={e=>setSettings({...settings,address:e.target.value})}/></div></div>
    ...
    <div className="space-y-6 min-w-0">
    <Card><CardContent className="p-6 space-y-4"><div className="flex items-center gap-2"><Palette className="h-5 w-5 text-primary"/><SectionLabel className="mb-0">{t("set_appearance_section")}</SectionLabel></div><div className="grid grid-cols-3 gap-4"><div><Label>{t("set_theme")}</Label><Select value={settings.theme} onChange={e=>handleThemeChange(e.target.value)}><option value="light">{t("set_theme_light")}</option><option value="dark">{t("set_theme_dark")}</option></Select></div><div><Label>{t("set_language")}</Label><Select value={lang} onChange={e=>handleLangChange(e.target.value)}><option value="en">{t("set_lang_english")}</option><option value="ml">{t("set_lang_malayalam")}</option></Select></div><div><Label>{t("set_animations")}</Label><Select value={fxPref} onChange={e=>setFxPref(e.target.value as "auto"|"reduced"|"full")}><option value="auto">{t("set_anim_auto")}</option><option value="reduced">{t("set_anim_reduced")}</option><option value="full">{t("set_anim_full")}</option></Select><p className="text-xs text-muted mt-1">{t("set_anim_hint")}</p></div></div></CardContent></Card>
    ...
    <Card><CardContent className="p-6 space-y-3"><div className="flex items-center gap-2"><Info className="h-5 w-5 text-primary"/><SectionLabel className="mb-0">{t("set_about_section")}</SectionLabel></div><div className="text-sm"><span className="text-text-tertiary">{t("set_app_version")}</span><div className="font-medium">{appInfo?.version || "—"}</div></div><div className="pt-1 border-t border-border/60"><div className="flex items-center gap-2 flex-wrap"><Button variant="secondary" onClick={checkUpdatesNow} disabled={updState==="checking"}>{updState==="checking"?<RefreshCw className="h-4 w-4 animate-spin"/>:<RefreshCw className="h-4 w-4"/>}{updState==="checking"?t("upd_checking"):t("upd_check_now")}</Button>{updState==="latest"&&<span className="text-xs text-text-tertiary">{t("upd_up_to_date")}</span>}{updState==="new"&&<span className="text-xs font-medium text-primary inline-flex items-center gap-1">{t("upd_found_new")}{updVersion?` (v${updVersion})`:""}{upd.state==="downloaded"?<button className="underline" onClick={upd.install}>{t("upd_restart")}</button>:upd.state==="downloading"?<span>{t("upd_downloading")}&nbsp;{upd.percent}%</span>:upd.state==="failed"?(updHasDl&&<button className="underline" onClick={()=>window.mms.updates.openDownload().catch(()=>{})}>{t("upd_download")}</button>):<button className="underline" onClick={upd.start}>{t("upd_download")}</button>}<button className="underline" onClick={()=>window.mms.updates.openReleasePage().catch(()=>{})}>{t("upd_open")}</button></span>}{updState==="error"&&<span className="text-xs text-rose-600">{t("upd_check_failed")}</span>}</div><p className="text-xs text-text-tertiary mt-1.5">{t("upd_monthly_note")}</p></div></CardContent></Card>
    </div>
    </div>
