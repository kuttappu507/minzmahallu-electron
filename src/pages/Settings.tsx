  ...
  return (
    <div className="p-6 max-w-4xl space-y-6">
      ...
      <Card><CardContent className="p-6 space-y-4"><div className="flex items-center gap-2"><Wallet className="h-5 w-5 text-primary" /><SectionLabel className="mb-0">{t("set_financial_section")}</SectionLabel></div><div className="grid grid-cols-3 gap-4">
        <div><Label>{t("set_financial_year_start")}</Label><Input value={settings.financial_year_start} placeholder="MM-DD" onChange={(e) => setSettings({ ...settings, financial_year_start: e.target.value })} /></div>
      ...
      <Card><CardContent className="p-6 space-y-4"><div className="flex items-center gap-2"><Database className="h-5 w-5 text-primary" /><SectionLabel className="mb-0">{t("set_backup_section")}</SectionLabel></div><div className="space-y-4">
        <div className="flex items-center justify-between"><div><Label className="mb-0">{t("set_auto_backup")}</Label><p className="text-xs text-text-tertiary mt-1">{t("ui_auto_backup_desc")}</p></div><Switch checked={settings.auto_backup} onCheckedChange={(v) => setSettings({ ...settings, auto_backup: v })} /></div>
        <div className="w-48"><Label>{t("set_backup_interval")}</Label><Input type="number" value={settings.backup_interval_hours} onChange={(e) => setSettings({ ...settings, backup_interval_hours: Number(e.target.value) })} /></div>
      </div></CardContent></Card>
