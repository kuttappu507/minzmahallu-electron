...
export function Settings(){
  ...
  return <div className="p-6 space-y-6">
    ...
    <Card><CardContent className="p-6 space-y-4"><div className="flex items-center gap-2"><Wallet className="h-5 w-5 text-primary"/><SectionLabel className="mb-0">{t("set_financial_sub_section")}</SectionLabel></div><div className="grid grid-cols-4 gap-4"><div><Label>{t("set_financial_year_start")}</Label><Input value={settings.financial_year_start} onChange={e=>setSettings({...settings,financial_year_start:e.target.value})}/></div><div><Label>{t("set_currency_symbol")}</Label><Input value={settings.currency_symbol} onChange={e=>setSettings({...settings,currency_symbol:e.target.value})}/></div><div><Label>{t("set_sub_frequency")}</Label><Select value={settings.subscription_frequency} onChange={e=>setSettings({...settings,subscription_frequency:e.target.value as "Monthly"|"Quarterly"})}><option value="Monthly">{t("set_sub_monthly")}</option><option value="Quarterly">{t("set_sub_quarterly")}</option></Select></div><div><Label>{settings.subscription_frequency === "Quarterly" ? t("set_sub_amount_quarterly") : t("set_sub_amount_monthly")}</Label><Input type="number" min="0" value={settings.subscription_monthly_amount} onChange={e=>setSettings({...settings,subscription_monthly_amount:Number(e.target.value)})}/></div></div><p className="text-xs text-text-tertiary">{t("set_sub_freq_hint")}</p></CardContent></Card>
