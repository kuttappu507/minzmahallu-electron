...
export function TokenEvents() {
  ...
  const [form, setForm] = useState({ event_name: "", event_type: "general", event_date: "", event_time: "", venue: "", description: "" });
  ...
  const openCreate = () => { setEditingId(null); setForm({ event_name: "", event_type: "general", event_date: "", event_time: "", venue: "", description: "" }); setDialogOpen(true); };
  const openEdit = (event: any) => { setEditingId(event.id); setForm({ event_name: event.event_name || "", event_type: event.event_type || "general", event_date: event.event_date || "", event_time: event.event_time || "", venue: event.venue || "", description: event.description || "" }); setDialogOpen(true); };
  const saveEvent = async () => {
    if (!form.event_name.trim() || !form.event_date) { toast.error(ml ? "ഇവന്റ് പേരും തീയതിയും ആവശ്യമാണ്" : "Event name and date are required"); return; }
    setSaving(true);
    try {
      if (editingId) await window.mms.tokens.updateEvent(editingId, form);
      else await window.mms.tokens.createEvent(form);
      toast.success(ml ? "ഇവന്റ് സേവ് ചെയ്തു" : "Event saved"); setDialogOpen(false); await load();
    } catch (e: any) { toast.error(e.message || (ml ? "ഇവന്റ് സേവ് ചെയ്യാനായില്ല" : "Failed to save event")); }
    finally { setSaving(false); }
  };
  const deleteEvent = async (event: any) => {
    const ok = window.confirm(ml ? `${event.event_name} ഇല്ലാതാക്കണോ? ഈ ഇവന്റിന്റെ ടോക്കണുകളും ഇല്ലാതാകും.` : `Delete ${event.event_name}? All tokens belonging to this event will also be deleted.`);
    if (!ok) return;
    try { await window.mms.tokens.removeEvent(event.id); toast.success(ml ? "ഇവന്റ് ഇല്ലാതാക്കി" : "Event deleted"); await load(); }
    catch (e: any) { toast.error(e.message || (ml ? "ഇവന്റ് ഇല്ലാതാക്കാനായില്ല" : "Failed to delete event")); }
  };

  return <div className="view view-enter">
    <div className="vhead"><span className="modic t-pink"><CalendarDays size={22} /></span><div><h1>{ml ? "ടോക്കൺ ഇവന്റുകൾ" : "Token Events"}</h1><div className="vs">{ml ? "ഇവന്റ് തിരഞ്ഞെടുക്കുകയും അതിന്റെ ടോക്കണുകൾ കൈകാര്യം ചെയ്യുകയും ചെയ്യുക" : "Choose an event, then manage its issued tokens."}</div></div><div className="vr"><Button onClick={openCreate}><Plus size={14} />{ml ? "പുതിയ ഇവന്റ്" : "New Event"}</Button><Button variant="secondary" onClick={load}><RefreshCw size={14} />{t("action_refresh")}</Button></div></div>
    ...
    <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={editingId ? (ml ? "ഇവന്റ് തിരുത്തുക" : "Edit Event") : (ml ? "പുതിയ ഇവന്റ്" : "New Event")}>
      <div className="dlg-pad space-y-4"><div><Label>{ml ? "ഇവന്റ് പേര്" : "Event Name"}</Label><Input value={form.event_name} onChange={e => setForm({ ...form, event_name: e.target.value })} autoFocus /></div><div><Label>{ml ? "ഇവന്റ് തരം" : "Event Type"}</Label><Select value={form.event_type} onChange={e => setForm({ ...form, event_type: e.target.value })}><option value="general">{ml ? "പൊതുവായത്" : "General"}</option><option value="eid">{ml ? "ഈദ്" : "Eid"}</option><option value="ramadan">{ml ? "റമദാൻ" : "Ramadan"}</option><option value="welfare">{ml ? "ക്ഷേമം" : "Welfare"}</option><option value="other">{ml ? "മറ്റുള്ളവ" : "Other"}</option></Select></div><div className="grid grid-cols-2 gap-3"><div><Label>{ml ? "തീയതി" : "Date"}</Label><Input type="date" value={form.event_date} onChange={e => setForm({ ...form, event_date: e.target.value })} /></div><div><Label>{ml ? "സമയം" : "Time"}</Label><Input type="time" value={form.event_time} onChange={e => setForm({ ...form, event_time: e.target.value })} /></div></div><div><Label>{ml ? "സ്ഥലം" : "Venue"}</Label><Input value={form.venue} onChange={e => setForm({ ...form, venue: e.target.value })} /></div><div><Label>{ml ? "വിവരണം" : "Description"}</Label><Textarea rows={3} value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} /></div><div className="dlg-actions"><Button variant="secondary" onClick={() => setDialogOpen(false)} disabled={saving}>{t("action_cancel")}</Button><Button onClick={saveEvent} disabled={saving}>{saving ? t("ui_saving") : (ml ? "സേവ്" : "Save Event")}</Button></div></div>
    </Dialog>
  </div>;
}
