...
export async function buildTokenSheetHtml(tokenList: any[], event: any, settings: any = {}, mode: TokenPrintMode = "color"): Promise<string> {
  ...
  const labels = ml ? {
    card:"കാർഡ് നമ്പർ", family:"കുടുംബം", familyNo:"കുടുംബ നമ്പർ", ward:"വാർഡ് നമ്പർ", head:"കുടുംബനാഥൻ", venue:"സ്ഥലം", event:"ഇവന്റ്", time:"സമയം"
  } : {
    card:"CARD NO", family:"FAMILY", familyNo:"FAMILY NO", ward:"WARD NO", head:"HOUSE HEAD", venue:"VENUE", event:"Event", time:"Time"
  };
  ...
  for(let i=0;i<tokenList.length;i+=12) pages.push(`<section class="page">${tokenList.slice(i,i+12).map((t:any)=>`
    <article class="card">
      ...
      <div class="mid">
        ...
        <div class="family"><span class="lbl">${labels.head}</span><h2>${esc(t.house_head_name || t.house_name || "—")}</h2><h3>${esc(t.house_name || t.family_number || "")}</h3></div>
        <div class="regs"><div class="r1"><small>${labels.familyNo}</small><b>${esc(t.family_number || "—")}</b></div><div class="r2"><small>${labels.ward}</small><b>${esc(t.ward || "—")}</b></div></div>
      </div>
