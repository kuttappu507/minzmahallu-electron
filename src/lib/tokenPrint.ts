...
export function buildTokenSheetHtml(tokenList: any[], event: any, settings: any = {}, mode: TokenPrintMode = "color"): string {
    ...
    <article class="card">
      ...
      <div class="mid">
        ...
        <div class="family">
          ...
          <h2>${esc(t.house_name || t.family_number || "—")}</h2>
          <h3>${esc(t.family_number || t.house_number || "")}</h3>
        </div>
        <div class="regs">
          <div class="r1"><small>FAMILY NO</small><b>${esc(t.family_number || "—")}</b></div>
          <div class="r2"><small>WARD NO</small><b>${esc(t.ward || "—")}</b></div>
        </div>
      </div>
      <footer class="event">
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><style>
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .head h1{font-size:10.5pt;font-weight:700;letter-spacing:.4px;text-transform:uppercase;line-height:1.15}
    .head p{font-size:5.9pt;opacity:.92;margin-top:.5mm;line-height:1.2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .mid{flex:1;display:flex;align-items:stretch;gap:2mm;padding:1mm 2mm;background:#fff;overflow:hidden}
    .token{width:15.5mm;background:${p.tokenBg};border:.35mm solid ${p.tokenLine};border-radius:2mm;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:.6mm;flex:none}
    .token small{font-size:4.6pt;font-weight:800;letter-spacing:1.2px;color:${p.eventTime}}
    .token b{font-size:12pt;color:${p.tokenNum};letter-spacing:.5px}
    .family{flex:1;text-align:center;min-width:0;display:flex;flex-direction:column;justify-content:center;gap:.5mm}
    .family .lbl{font-size:4.6pt;letter-spacing:1.6px;color:#a8a29e;font-weight:700}
    .family h2{font-size:11.5pt;color:#1e293b;font-weight:800;line-height:1.15;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .family h3{font-size:8pt;color:#64748b;font-weight:600;letter-spacing:1px;text-transform:uppercase;line-height:1.2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .regs{width:19.5mm;display:flex;flex-direction:column;justify-content:center;gap:1.1mm;flex:none}
    .regs div{background:${p.chipBg};border-radius:1.5mm;padding:.9mm 1.3mm;min-width:0}
    .regs .r1{border-left:.7mm solid ${p.acc1}} .regs .r2{border-left:.7mm solid ${p.acc2}}
    .regs small{display:block;font-size:4.6pt;letter-spacing:.55px;color:#64748b;font-weight:700;white-space:nowrap}
    .regs b{font-size:8.6pt;color:#1e293b;white-space:nowrap}
    .event{background:${p.eventBg};border-top:.3mm solid ${p.eventLine};text-align:center;padding:1.3mm 2mm 1.5mm}
    .event h4{font-size:11.5pt;font-weight:800;color:${p.eventName};letter-spacing:.8px;text-transform:uppercase;line-height:1.15;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .event p{font-size:6.4pt;color:#475569;margin-top:.6mm;line-height:1.2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .event p b{color:${p.eventTime}} .event .sep{color:${p.sep};margin:0 1.2mm}
    ${mode === "bw" ? `.head{background:#111!important;border-bottom-color:#111!important}.token b,.family h2,.event h4{color:#111!important}.event{background:#f4f4f4!important;border-top-color:#999!important}.token{background:#f3f3f3!important}.regs div{background:#f1f1f1!important}` : ""}
