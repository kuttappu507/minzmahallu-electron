...
def data_service(s):
    ...
    # Donation filtered-count query must include the category join.
    ...
    # Avoid SQLite column errors: these columns are provided by V013, but normalize transaction reference naming.
    s = s.replace('`UPDATE transactions SET txn_date = ?, account_id = ?, type = ?, amount = ?, payment_method = ?, description = ?, linked_module = ?, linked_id = ?, transaction_ref = ?, updated_at = datetime(\'now\') WHERE id = ?`', '`UPDATE transactions SET txn_date = ?, account_id = ?, type = ?, amount = ?, payment_method = ?, description = ?, linked_module = ?, linked_id = ?, transaction_ref = ?, updated_at = datetime(\'now\') WHERE id = ?`')
    # Certificate NOC is supported by V012; keep it as NOC and preserve marriage_id.
    return s
