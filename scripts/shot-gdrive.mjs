...
const HOME = fs.mkdtempSync(path.join(os.tmpdir(), "mms-shot-gdrive-"));
const DEBUG_PORT = 9379;
const OUT = "/home/z/my-project/download/settings-gdrive.png";
...
try {
  ...
  await sleep(2500);
  // Fill a sample client id so the Connect button is enabled in the shot.
  await conn.evaluate(`(() => {
    const inputs = [...document.querySelectorAll('input')];
    const idInput = inputs.find(i => /apps.googleusercontent.com/.test(i.placeholder || ''));
    if (idInput) {
      const desc = Object.getOwnPropertyDescriptor(Object.getPrototypeOf(idInput), 'value');
      desc.set.call(idInput, '1234567890-abc123.apps.googleusercontent.com');
      idInput.dispatchEvent(new Event('input', { bubbles: true }));
    }
  })()`);
  await sleep(400);
  const probe = await conn.evaluate(`(() => {
    const card = [...document.querySelectorAll('.card')].find(c => /Google Drive backup|ഗൂഗ്ൾ ഡ്രൈവ് ബാക്കപ്പ്/.test(c.textContent || ''));
    if (card) card.scrollIntoView({ block: 'center' });
    const text = document.body.innerText;
    const gdriveBlock = [...document.querySelectorAll('.card > div')].find(d => /Google Drive backup/.test(d.textContent || ''));
    if (gdriveBlock) gdriveBlock.scrollIntoView({ block: 'center' });
    return { hasGdriveText: !!text.match(/Google Drive backup/),
             hasClientIdField: [...document.querySelectorAll('label')].some(l => /Google Client ID/.test(l.textContent || '')),
             hasConnectBtn: [...document.querySelectorAll('button')].some(b => /Connect Google account/.test(b.textContent || '')),
             hasHelpLink: [...document.querySelectorAll('button')].some(b => /How to get the Client ID/.test(b.textContent || '')),
             connectDisabled: [...document.querySelectorAll('button')].filter(b => /Connect Google account/.test(b.textContent || ''))[0]?.disabled };
  })()`);
