...
-- ----------------------------------------------------------------------------
-- Meta: schema version tracking for migration support
-- ----------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS users (
    ...
    password_hash   TEXT    NOT NULL,            -- Argon2id / PBKDF2 encoded string
    password_salt   TEXT    NOT NULL,
    role            TEXT    NOT NULL CHECK (role IN (
                        'Administrator','President','Secretary','Treasurer',
                        'Imam','Staff','Auditor'
                    )),
    email           TEXT,
    phone           TEXT,
...
-- ----------------------------------------------------------------------------
-- Role-based permission matrix (fine-grained)
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS permissions (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    role            TEXT    NOT NULL,
    module          TEXT    NOT NULL,             -- e.g. 'family','member','accounting'
    action          TEXT    NOT NULL,             -- 'view','add','edit','delete','print','export'
    allowed         INTEGER NOT NULL DEFAULT 0,
    UNIQUE(role, module, action)
);

-- ----------------------------------------------------------------------------
-- Mahallu / Organization settings (single-row table)
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS settings (
    id              INTEGER PRIMARY KEY CHECK (id = 1),
    mahallu_name    TEXT    NOT NULL DEFAULT 'Minz Mahallu Management',
    address         TEXT,
    phone           TEXT,
    email           TEXT,
    logo_path       TEXT,
    seal_path       TEXT,
    financial_year_start TEXT NOT NULL DEFAULT '04-01',  -- MM-DD (April 1)
...
CREATE TABLE IF NOT EXISTS subscription_plans (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    name            TEXT    NOT NULL UNIQUE,        -- Monthly / Yearly / Special
...
INSERT OR IGNORE INTO subscription_plans (name, frequency, default_amount) VALUES
    ('Monthly Subscription','Monthly',100),
    ('Yearly Subscription','Yearly',1200),
    ('Special Subscription','OneTime',0);
...
CREATE TABLE IF NOT EXISTS subscriptions (
    ...
    period_start    TEXT,                            -- billing period start (YYYY-MM-DD)
    period_end      TEXT,
    amount          REAL    NOT NULL,
    amount_paid     REAL    NOT NULL DEFAULT 0,
    payment_date    TEXT,
    receipt_number  TEXT    UNIQUE,
    payment_method  TEXT    CHECK (payment_method IN ('Cash','Cheque','UPI','Bank Transfer','Card','Other')),
    transaction_ref TEXT,
    status          TEXT    NOT NULL DEFAULT 'Pending' CHECK (status IN ('Paid','Pending','Overdue','Partial')),
    collected_by    INTEGER,                          -- user id
    remarks         TEXT,
    created_at      TEXT    NOT NULL DEFAULT (datetime('now')),
    updated_at      TEXT    NOT NULL DEFAULT (datetime('now')),
    FOREIGN KEY (family_id) REFERENCES families(id) ON DELETE RESTRICT,
...
INSERT OR IGNORE INTO donation_categories (name) VALUES
    ('General Donation'),('Masjid Donation'),('Building Fund'),
    ('Education Fund'),('Medical Fund');
...
CREATE TABLE IF NOT EXISTS ledger_accounts (
    ...
    category        TEXT,                              -- Salary, Electricity, Rent, etc.
    is_active       INTEGER NOT NULL DEFAULT 1
);
INSERT OR IGNORE INTO ledger_accounts (code,name,type,category) VALUES
    ('INC-SUB','Subscription Income','Income','Subscription'),
    ('INC-DON','Donation Income','Income','Donation'),
    ('INC-RENT','Rent Income','Income','Rent'),
    ('INC-OTH','Other Income','Income','Other'),
    ('EXP-SAL','Salary Expense','Expense','Salary'),
    ('EXP-ELC','Electricity Expense','Expense','Electricity'),
    ('EXP-WAT','Water Expense','Expense','Water'),
    ('EXP-MAINT','Maintenance Expense','Expense','Maintenance'),
    ('EXP-WEL','Welfare Expense','Expense','Welfare'),
    ('EXP-OTH','Other Expense','Expense','Other');
...
CREATE TABLE IF NOT EXISTS transactions (
    ...
    reference       TEXT,
    description     TEXT,
    linked_module   TEXT,                              -- 'subscription','donation','welfare', etc.
    linked_id       INTEGER,
    receipt_number  TEXT,
    created_by      INTEGER,
    created_at      TEXT    NOT NULL DEFAULT (datetime('now')),
...
CREATE TABLE IF NOT EXISTS marriages (
    id                  INTEGER PRIMARY KEY AUTOINCREMENT,
    marriage_number     TEXT    NOT NULL UNIQUE,
...
CREATE TABLE IF NOT EXISTS deaths (
    id                  INTEGER PRIMARY KEY AUTOINCREMENT,
    death_number        TEXT    NOT NULL UNIQUE,
...
-- ----------------------------------------------------------------------------
-- Documents (attachments, polymorphic)
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS documents (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    linked_module   TEXT    NOT NULL,                  -- 'family','member','marriage','death','welfare'
    linked_id       INTEGER NOT NULL,
    file_name       TEXT    NOT NULL,
    file_path       TEXT    NOT NULL,                  -- relative to data dir
    file_type       TEXT,                              -- mime / ext
    file_size       INTEGER,
    uploaded_by     INTEGER,
...
CREATE TABLE IF NOT EXISTS audit_log (
    ...
    user_id         INTEGER,
    username        TEXT,
    action          TEXT    NOT NULL,                  -- LOGIN, LOGOUT, ADD, EDIT, DELETE, PRINT, EXPORT, BACKUP, RESTORE
    module          TEXT,
    entity_id       INTEGER,
    description     TEXT,
    ip_address      TEXT,
...
-- ----------------------------------------------------------------------------
-- Sessions (for tracking active sessions, optional server-side invalidation)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- View: Defaulters (families with overdue / pending monthly subscription)
-- ----------------------------------------------------------------------------
...
SELECT f.id AS family_id, f.family_number, f.house_name, f.phone,
       ...
       SUM(s.amount - s.amount_paid) AS due_amount
FROM families f
...
-- ----------------------------------------------------------------------------
-- View: Member directory (joined with family)
-- ----------------------------------------------------------------------------
CREATE VIEW IF NOT EXISTS v_member_directory AS
SELECT m.id AS member_id, m.member_code, m.name, m.arabic_name, m.gender,
       m.date_of_birth, m.age, m.mobile, m.email, m.occupation, m.blood_group,
       m.marital_status, m.status,
       f.id AS family_id, f.family_number, f.house_name, f.ward, f.area
FROM members m
